...
const ConfigUsers = ({ onOpenQuizzes }: Props) => {
  ...
  const handleAdminResetPassword = (userId: string) => {
    ...
    socket.emit(EVENTS.MANAGER.ADMIN_RESET_PASSWORD, { userId, newPassword })
  }
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="min-h-0 flex-1 space-y-2 overflow-auto p-0.5">
        {users?.map((u) => (
          <div
            key={u.id}
            className="flex flex-col w-full rounded-md outline outline-gray-300 bg-white overflow-hidden"
          >
                ...
                {currentUser?.role === "admin" && u.id !== currentUser?.id && (
                  <>
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 text-xs font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-md transition shrink-0"
                    >
                      Reset Pass
                    </button>
                  </>
            ...
            {resettingUserId === u.id && (
              <div className="flex items-center gap-2 px-3 pb-3 border-t border-gray-100 pt-3 bg-gray-50/50">
                <input
                  type="password"
                  value={newPassword}
                  onChange={(e) => setNewPassword(e.target.value)}
                  placeholder="New password (min 4 chars)"
                  className="flex-1 px-3 py-1 border border-gray-300 rounded text-xs bg-white focus:outline-none"
                />
                <button
                ...
                <button
                  ...
                  className="px-3 py-1 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded text-xs font-bold transition"
                >
                  Cancel
                </button>
              </div>
